...
function hasCollidedWithSnake() {
  ...
  */
for (var s = snake.body.length - 1; s > 1; s-- ){
  if (snake.body[s].row === snake.head.row && snake.body[s].column === snake.head.column){
    return true;
  }
}
...
function getRandomAvailablePosition() {
  ...
  while (!spaceIsAvailable) {
    ...
    for (var a = snake.body.length - 1; a > 0; a-- ) {
    if (randomPosition.column === snake.body[a].column && randomPosition.row === snake.body[a].row) {
